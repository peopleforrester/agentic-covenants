...
- Kyverno 1.13+ or OPA Gatekeeper for admission control.
...
- [`kyverno-verify-image-signatures.yaml`](./kyverno-verify-image-signatures.yaml) — Kyverno ClusterPolicy verifying cosign signatures from a specific GitHub OIDC subject and verifying the SPDX attestation. Requires Kyverno 1.13+ for the `attestors`/`entries`/`keyless` block shape.
