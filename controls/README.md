...
1. **Control.** One-line summary from [`MATRIX.md`](../framework/MATRIX.md).
...
5. **Common mistakes.** Bypasses from [`BYPASSES.md`](../framework/BYPASSES.md) that defeat this cell if you do it wrong.
...
Build server-side first, client-side second, in-agent last. Reason: server-side does not depend on the agent or operator behaving correctly. If you only have time for one row, build the bottom one. The week-by-week rollout sequence is at the end of [`MATRIX.md`](../framework/MATRIX.md).
