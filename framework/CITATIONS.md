...
- **OWASP Top 10 for LLM Applications 2025** (released November 2024). LLM01 Prompt Injection through LLM10 Unbounded Consumption. **Superseded: a 2026 edition, v1.0, was published 3 August 2026** (<https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/>, verified 2026-09-19). Every `LLMnn` citation in this repo is against the **2025** numbering. That is a live problem rather than a cosmetic one, because the 2026 edition is reported to have retained all ten categories while **reordering** them, so an ID can now denote a different risk than the cell citing it intends. The remapping is tracked in [#12](https://github.com/peopleforrester/agentic-covenants/issues/12) and is deliberately not applied here until the 2026 item list is confirmed against the published document rather than secondary coverage.
