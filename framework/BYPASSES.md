...
### The MCP advisory wave, and the week the official SDKs joined it (August to September 2026)

- **Measured**, not estimated: **46 MCP-related advisories** in the GitHub Advisory Database published between 1 August and 19 September 2026, with **16 landing on 2026-08-25 alone**. Counted by querying reviewed advisories in that window and matching on package name and summary.
- **Layer affected**: Supply chain, and it removes a mitigation this file previously treated as sound.
- **What changed, and it is not the volume.** Until now the MCP CVEs in this file were community servers, and the honest mitigation was to allowlist, hash-pin, and prefer official implementations. That last clause no longer holds. Advisories are now filed against the **official SDKs**:
  - **CVE-2026-63127**, `rmcp`, the official Rust SDK, high, CVSS 8.2, 2026-09-16. Missing Resource field validation in OAuth Protected Resource Metadata discovery.
  - **CVE-2026-63128**, `rmcp`, high, 7.5. An unauthenticated permanent session-table leak in the Streamable HTTP transport, which is remote denial of service against the transport the `2026-07-28` revision moved everyone toward.
  - **CVE-2026-53965**, `mcp/sdk`, the official PHP SDK, high, CVSS v4 8.7, 2026-08-19. A client buffer grows without bound when a server withholds the event delimiter, so a hostile server exhausts the client.
- **Three patterns worth separating from the list:**
  1. **DNS rebinding defeats "it only listens on localhost."** `mysql-mcp-server` **CVE-2026-59971** is **CVSS 10.0**: missing Origin and Host validation in the SSE transport yields unauthenticated SQL execution, reachable by rebinding a hostname at a browser the operator is already using. An MCP server bound to loopback is not therefore unreachable, and a local-only bind is not an access control. This file had no rebinding entry before this one, which was a gap.
  2. **Prompt injection reaching remote code execution with nothing in between.** Flowise **CVE-2026-70477**, CVSS v4 9.5, is titled *CSV Agent Prompt Injection Remote Code Execution*. This is the chain the framework says it cannot prevent and can only bound, appearing as a single numbered vulnerability rather than as a theory.
  3. **A patch bypass of a patch already in this corpus.** **CVE-2026-69263** defeats the MCP environment-variable blocklist added for CVE-2025-8943 by going through `npm_config_yes`. The blocklist was present, maintained, and enumerating; the attacker used a name not on it. That is the same shape as [the read-only validation bypass](#claude-code-read-only-validation-bypass-via-ifs-and-short-flags-cve-2025-66032): a control that answers confidently and wrongly.
- **Lesson, and it is uncomfortable for the supply-chain cell.** [`controls/supply-chain/client-side/mcp-allowlist.json`](../controls/supply-chain/client-side/mcp-allowlist.json) pins **which** server runs and verifies **that** it is the one you chose. It says nothing about whether that server is safe, and hash-pinning an official SDK carrying CVE-2026-63127 pins the vulnerability precisely. Allowlisting answers identity, not soundness. The controls that still bound this are the ones that do not care what the server is: the egress allowlist in [`controls/supply-chain/server-side/cilium-mcp-fqdn-egress.yaml`](../controls/supply-chain/server-side/cilium-mcp-fqdn-egress.yaml), the sandbox mount set, and the credential the server can actually reach. At least one entry in the wave, `omniroute` **CVE-2026-88062**, CVSS v4 9.5, **has no patched version**, which is the case where bounding is the only option available.
...
When you populate a cell, you are buying the cell's strength minus the cell's bypass surface. A reviewer's first job is to ensure the bypasses are documented and consciously accepted. **An undocumented control that fails to its bypass is the worst kind of governance theater.**
